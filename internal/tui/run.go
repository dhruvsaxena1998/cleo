...
func Run(c *cli.Ctx) error {
	...
	// (e.g. a tmux that doesn't pass COLORTERM), where distinct theme colours
	// collapse to the same approximation and popup borders stop reading as
	// theme-specific. Modern terminals universally support 24-bit colour; those
	// that don't silently approximate the sequences, no worse than ANSI256.
